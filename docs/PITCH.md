# Sigil — Pitch & Competitive Positioning
...
```json
{
  ...
  "name": "tdd-feature",
  "nodes": {
    "planner":     { "adapter": "claude-sdk", "model": "claude-opus-4-6",   "role": "Planner",     "prompt": "..." },
...
```bash
sigil run ./tdd-feature.json "add OAuth2 login with GitHub provider"
```

### 3. What happens

1. Sigil validates the graph, probes adapters (API keys, binaries), and computes critical-path priorities
...
7. On completion or failure: checkpoint saved to `.sigil/runs/<id>.json` — resumable with `sigil resume`
...
| | Sigil | LangGraph | CrewAI | AutoGen | OpenAI Agents | Composio |
|---|---|---|---|---|---|---|
...
**Sigil's approach:** Each node's `Promise.finally(wake)` resolves a shared wakeup promise. The main loop blocks on this promise, then immediately computes the next set of ready nodes. No polling interval. No batch barrier. A fast node unblocks its successors instantly, even if sibling nodes are still running.
...
**Sigil's approach:** At workflow start, compute the longest path from each node to workflow completion (using historical `durationMs` or a default weight). Ready nodes are sorted by descending critical-path weight. The bottleneck node always starts first.
...
**Sigil's approach:** `LazyWorktreeManager` creates git worktrees on-demand (only for nodes with write-capable tools) using sparse checkout — only the directories the node needs are checked out. A `WorktreeMutex` serializes git operations to avoid `.git/index.lock` contention. At fan-in points, worktrees are automatically merged back.
...
**Sigil's approach:** `EventFanOut` gives each client a `RingBuffer<string>` (1024 events). `emit()` pushes a pre-serialized JSON string into each buffer — O(n) with zero I/O. A 16ms interval timer drains each buffer, coalesces consecutive `text_delta` events, and batch-sends. Clients exceeding `maxBufferedAmount` are disconnected.
...
- **Gate script environment:** Whitelist-only env vars (`PATH`, `HOME`, `SHELL`, `SIGIL_*`)
