...
**Shared adapter helpers** — the four stream-json CLI adapters (`claude-cli`, `codex-cli`, `cursor-cli`, `gemini-cli`) also share `adapters/constants.ts` (stall/getResult timings + `exitCodeToSygilError` exit-code→`SygilErrorCode` mapping), `adapters/ndjson-line-decoder.ts` (UTF-8-safe chunk→line decoder), and `adapters/session.ts` (`makeAgentSession()` outer envelope). New CLI adapters should reuse these before rolling their own.
